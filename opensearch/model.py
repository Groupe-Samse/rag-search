...
def search_model(open_search_client, model_name):
    endpoint = "/_plugins/_ml/models/_search"
    response = open_search_client.transport.perform_request(
        method="GET",
        url=endpoint,
...
def register_and_deploy_ml_model(opensearch_client, model_payload, model_name):
    existing_model = search_model(opensearch_client, model_name)
    ...
    while get_model_id_from_task_id(opensearch_client, task_id) is None:
        print("Waiting for model to be ready...")
        time.sleep(1)
    return get_model_id_from_task_id(opensearch_client, task_id)


def get_model_id_from_task_id(opensearch_client, task_id):
...
def put_ingest_pipeline(opensearch_client, model_id):
    pipeline_body = {
    ...
    opensearch_client.ingest.put_pipeline("test-pipeline-local-model", body=pipeline_body)


def search_connector(opensearch_client, connector_name):
...
def create_connector(opensearch_client, model_name, openai_key):
    ...
    response = opensearch_client.transport.perform_request(method="POST", url=endpoint, body=connector_config)
    return response["connector_id"]


def search_agent(opensearch_client, agent_name):
...
def register_agent(opensearch_client, agent_name, index_name, sentence_transformer_model_id, gpt_model_id):
    ...
    response = opensearch_client.transport.perform_request(method="POST", url=endpoint, body=agent_config)
    return response["agent_id"]


def query_agent(opensearch_client, agent_id, question):
